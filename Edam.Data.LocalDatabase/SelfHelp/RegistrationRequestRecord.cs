...
public class RegistrationRequestRecord
{
        ...
        App.ApplicationRequestContext context, Int16 optionNo = 0)
  {
     ...
     result.Instance = new SelfHelp.RegistrationEmailVerifiedInfo();
        ...
        Edam.Security.Password password)
  {
     ...
     Session.OrganizationId = result.Instance.OrganizationId;
        ...
        String sessionId, Edam.LocaleLanguage language, Int16 groupNo)
  {
     ...
     results.Instance = new RegistrationIdsCodes();
     DataProvider p = null;
     ...
     try
     {
        ...
        if (p.OpenReader())
        {
           results.Instance.LocationCountries =
              DataReader.GetList<DataCodes.DataCodeInfo>(p.Reader);
           p.Reader.NextResult();
           results.Instance.LocationStates =
              DataReader.GetList<Locations.LocationStateInfo>(p.Reader);
           p.Reader.NextResult();
           results.Instance.Organizations =
              DataReader.GetList<DataCodes.DataCodeInfo>(p.Reader);
           p.Reader.NextResult();
           results.Instance.Applications =
              DataReader.GetList<DataCodes.DataCodeInfo>(p.Reader);
           p.Reader.NextResult();
        ...
        String sessionId, Edam.LocaleLanguage language, Int16 groupNo)
  {
     ...
     response.ResponseData = result.Success ? result.Instance : null;
